\section{Set up}
{GRASP Builder application can be executed in both operative systems, Windows and Linux.
The only prerequisite in order to work with this application is to have MATLAB installed.
\smallskip\\The only functionality not enabled in Windows is the execution of GRASP algorithm, since this application is desigend to work in CALCULA enviroment, but all the other funcionalities can be used, if the user prefers.}
\subsection{Windows}
{Since this application is not being installed with any executable or saved in registry, it is only necessary to unzip file \textit{GRASP\_Builder\_v200\_Windows.zip} in the desired folder.}
\begin{figure}[H]
\centering
...
\end{figure}
{In the unziped folder it will be located and executable called GRASP\_Builder.exe and it can be executed.}
\begin{figure}[H]
\centering
...
{Similar procedure needs to be followed in the Linux enviroment, but it all can be executed via command line. In this case, it is needed an extra step, since Linux needs to have specified and application as executable.
...
\begin{lstlisting}
    unzip GRASP_Builder_v200_Linux.zip
    cd GRASP_Builder_v200_Linux
