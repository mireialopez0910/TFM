...
{The first button will open a dialog to create a new project (\Cref{fig:CreateNewProject}). The second button will open a dialog that allows the user to open an existing project (\Cref{fig:OpenProject}). The third button will open a dialog that allows the user to import a project from a zip file (\Cref{fig:ImportProject}).\smallskip\\}
...
{In the project folder, it is necessary that the file "project.grasp" exists.} Without this file, the application will not be able to load the project and an error message will be displayed (\Cref{fig:ErrorOpen}).
\begin{figure}[H]
\centering
...
{Next to the File menu, the Settings button can be located. This button opens a dialog that allows the user to configure different options of the application \Cref{fig:Settings}.}
\begin{figure}[H]
\centering
...
{The different parameters that can be configured are separated by application configuration parameters and project configuration parameters. The application configuration parameters have the same value for all projects. For now, it only includes the installation directory of GRASP algorithm.\smallskip\\ 
Project configuration parameters are specific for each project. For now, it only includes the corresponding repository directories where the downloaded data will be stored for both EARLINET and AERONET sites, and the MATLAB output directory, where all the files created after running the corresponding scripts and GRASP algorithm will be created.}