...
\begin{enumerate} 
    \item Optical Depth\cite{aod_solar}: 
    \begin{itemize}
        \item Aerosol Optical Depth (AOD): (descripcion de los datos), with extension .lev15
        \item Spectral Deconvolution Algorithm (SDA) data: (descripcion de los datos), with extension .ONEILL\_lev15
    \end{itemize}       
    \item Aerosol Inversions\cite{inversions_aod_solar}:
    \begin{itemize}
        \item Inversion products:(descripcion de los datos), with extension .all
    \end{itemize}
    \item Raw Products Optical Depth\cite{raw_aod_solar};
    \begin{itemize}
        \item Raw Almucantar: (descripcion de los datos), with extension .alm
        \item Raw Polarized Almucantar: (descripcion de los datos), with extension .alp
    \end{itemize}
\end{enumerate}
\smallskip
