...
After testing and comparing the different options and the obtained responses, it becomes clear that the best option is to use the \texttt{/products/downloads} endpoint, which allows users to download the measurements in a compressed format, while filtering the data by type, date, and station name.\smallskip\\
...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
{When creating this class, consideration was given to the fact that the \texttt{DownloadProductByDateRangeAsync} method can be used at various points in the program with different configurations. It has also been implemented in such a way that adding new functionalities is straightforward using the same design pattern. For clarity and organisation, the \texttt{DataType} Enum has been created, since the different data types require different URL content to be downloaded. A simplification of this class can be found in Appendix \ref{ap:aeronet_service}.\smallskip\\}

{In this application, this class is only used to download the data files listed above, but other types could be downloaded by simply calling the \texttt{DownloadDataAsync} and \texttt{BuildUrl} methods.}
...
{To execute the MATLAB scripts from C\#, a method has been created that uses the \texttt{ProcessStartInfo} class, which is part of the standard library, to execute the MATLAB script. Appendix \ref{ap:run_matlab_script} shows the \texttt{RunMatlabScript} method that takes a string parameter containing the path to the script to be executed. The method uses the \texttt{Process} class to execute the script and it redirects the standard output and standard error to the console.\smallskip}
