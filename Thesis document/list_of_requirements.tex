...
{This first step is crucial for the correct result. It is needed to define a clear and well defined list of requirements. This defines the main scope of the project, the different features to be implemented and prevents from building something that is not desired. }
\smallskip
{This project needs to be able to fulfill the following requirements: }
\begin{enumerate} 
    ...
    \begin{enumerate}
      ...
      \begin{itemize} %no es descarreguen tots, revisar quin eliminar
        \item {Download Raw Almucantar products}
        \item {Download Raw Polarized Almucantar products}
        \item {Download Derived Inversion Products with Almucantar scenario}
        \item {Download Aerosol Optical Depth (AOD) products}
        \item {Download Total Optical Depth products based on AOD level}
      \end{itemize}
    \end{enumerate}
    \item {Filter Earlinet measurement files depending on the the data type: it is necessary that measurement has been done during day time, in order to have solar almucantar measurements. If not, these measurements should be discarded.}
    ...
    \begin{enumerate}
      ...
      \item {Create .yml configuration file containing corresponding .sdat file name and chosen configuration}
    \end{enumerate}
    ...
    \begin{itemize}
      ...
      \item {Output directory can be modified}
