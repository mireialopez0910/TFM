...
%! de la mesura que inclou dades sense despolarització del LIDAR i sense
%! polarització del fotòmetre perquè la funció WriteFiles_preGARRLiC_D1_L()
...
%!
%! * |measureID: [string]| -
%!      Identificador de la mesura per a la qual s'hi agafan les dades (*.nc).
%!
%! * |heightMin: [int]| -
...
%!
    ...
    try
        ...
        %! Si se li ha passat el paràmetre |isFigure = *True*| la funció representarà algunes gràfiques que permeten analitzar les dades carregades
        %! Per exemple, gràfiques amb RCS1064, RCS355 i RCS532
        if isFigure == true
            figure(1)
            semilogy(RCS1064*100000, Range1064, '-r'), hold on
            semilogy(RCS355, Range355, '-b')
            semilogy(RCS532, Range532, '-g'), hold off
        ...
        Rangelog = logspace(log10(heightMin),log10(heightMax), 60);        
        ...
        %! A continuació es realitza una normalització del perfil
        RCS1064log = RCS1064log./trapz(Rangelog,RCS1064log);
        RCS532log = RCS532log./trapz(Rangelog,RCS532log);
        RCS355log = RCS355log./trapz(Rangelog,RCS355log);
        ...
        %!
        fullFileName = fullfile(URL_output, ['GRASP_', location, '_', NombreEntrada2, '_D1_L']);
        save(fullFileName, 'Rangelog','RCS1064log','RCS532log','RCS355log','heightMax','heightMin','NombreEntrada2')
        ...
        diferencia = abs(numT1 - numAlm); % Calcula la diferencia absoluta entre las fechas
        [~, Ind] = min(diferencia); % Encuentra el índice de la fecha más cercana
        ...
        DayOftheYear = T1.Day_of_Year(1); % [updated] Coge solo el primer valor (los días no cambian)
        ...
        rad1640 = T2{Index1640,23:49}; rad1640 = [rad1640;fliplr(T2{Index1640,50:76})]; mrad1640 = mean(rad1640);
        rad1020 = T2{Index1020,23:49}; rad1020 = [rad1020;fliplr(T2{Index1020,50:76})]; mrad1020 = mean(rad1020);
        rad870 = T2{Index870,23:49}; rad870 = [rad870;fliplr(T2{Index870,50:76})]; mrad870 = mean(rad870);
        rad675 = T2{Index675,23:49}; rad675 = [rad675;fliplr(T2{Index675,50:76})]; mrad675 = mean(rad675);
        rad500 = T2{Index500,23:49}; rad500 = [rad500;fliplr(T2{Index500,50:76})]; mrad500 = mean(rad500);
        rad440 = T2{Index440,23:49}; rad440 = [rad440;fliplr(T2{Index440,50:76})]; mrad440 = mean(rad440);
        rad380 = T2{Index380,23:49}; rad380 = [rad380;fliplr(T2{Index380,50:76})]; mrad380 = mean(rad380);
        
        SAA = [3.5 4 5 6 7 8 10 12 14 16 18 20 25 30 35 40 45 50 60 70 80 90 100 120 140 160 180];
        SAA1640 = SAA; SAA1020 = SAA; SAA870 = SAA; SAA675 = SAA; SAA500 = SAA; SAA440 = SAA; SAA380 = SAA;
        ...
        Index1640 = find(isfinite(mrad1640));
        mrad1640 = mrad1640(Index1640);
        SAA1640 = SAA1640(Index1640);
        ...
        %! de la Terra al Sol
        DayAngle = 2*pi*(DayOftheYear-1)/365; % Ángulo para el día del año 
        EarthSunDist = 1/(1.00011+(0.034221*cos(DayAngle))+0.00128*sin(DayAngle)+0.000719*cos(2*DayAngle)+0.000077*sin(2*DayAngle)); % Distancia de la Tierra al Sol
        ...
        ground_height = 0; % Other variables / surface measurements
        
        %!
        %%! Escriptura preGARRLiC
        %! Una vegada recollit totes les dades s'envien a la funció
        %! |WriteFiles_preGARRLiC_D1_L(...)| encarregada d'escriure el fitxer
        %! de sortida SDAT. Si s'escriu correctament retornarà el nom
        %! de l'arxiu creat.
        
        [GARRLiC_file_name] = WriteFiles_preGARRLiC_D1_L(measureID, URL_output, latitude,longitude,ground_height,SZA380,SZA440,SZA500,...
            ...
            nRadiance1640,Rangelog,RCS355log,RCS532log,RCS1064log,NombreEntrada1,NombreEntrada2);
